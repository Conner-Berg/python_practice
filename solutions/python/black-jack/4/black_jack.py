...
def value_of_ace(card_one: str, card_two: str) -> int:
    ...
    hand_total = (ACE_HIGH if card_one == "A" else value_of_card(card_one)) + (
        ACE_HIGH if card_two == "A" else value_of_card(card_two)
    )
    return ACE_HIGH if hand_total + ACE_HIGH <= 21 else ACE_LOW
...
def is_blackjack(card_one: str, card_two: str) -> bool:
    ...
    if card_one == "A":
        return value_of_card(card_two) == FACE_CARD_VALUE
    if card_two == "A":
        return value_of_card(card_one) == FACE_CARD_VALUE
    return False
